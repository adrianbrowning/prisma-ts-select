...
describe("PostgreSQL distinct() helper", () => {
    ...
    describe("stringAgg(distinct(col), sep)", () => {
        function createQuery() {
            return prisma.$from("User")
                .groupBy(["User.id"])
                .select(({ stringAgg, distinct }) => stringAgg(distinct("User.name"), ", "), "names");
        }

        it("should match SQL", () => {
            expectSQL(createQuery().getSQL(),
                `SELECT STRING_AGG(DISTINCT "User"."name", ', ') AS ${dialect.quote("names", true)} FROM ${dialect.quote("User")} GROUP BY "User"."id";`);
        });
    ...
    describe("arrayAgg(distinct(col))", () => {
        function createQuery() {
            return prisma.$from("User")
                .groupBy(["User.id"])
                .select(({ arrayAgg, distinct }) => arrayAgg(distinct("User.name")), "names");
        }

        it("should match SQL", () => {
            expectSQL(createQuery().getSQL(),
                `SELECT ARRAY_AGG(DISTINCT "User"."name") AS ${dialect.quote("names", true)} FROM ${dialect.quote("User")} GROUP BY "User"."id";`);
        });
