...
describe("PostgreSQL string dialect fns", () => {
  ...
  describe("substring(col, start, len)", () => {
    ...
    it("should return first 5 chars", async () => {
      const result = await createQuery().run();
      const values = result.map(r => r.s).sort((a, b) => String(a).localeCompare(String(b)));
  ...
  describe("length(col)", () => {
    ...
    it("type: number", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ elen: number; }>>>);
    });
  ...
  describe("left(col, n)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ left }) => left("User.email", 5), "prefix")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ prefix: string; }>>>);
    });
  });
  ...
  describe("right(col, n)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ right }) => right("User.email", 11), "domain")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ domain: string; }>>>);
    });
  });
  ...
  describe("repeat(col, n)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ repeat }) => repeat("User.name", 2), "doubled")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ doubled: string; }>>>);
    });
  });
  ...
  describe("reverse(col)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ reverse }) => reverse("User.name"), "rev")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ rev: string; }>>>);
    });
  });
  ...
  describe("lpad(col, len, pad)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ lpad }) => lpad("User.name", 10, "*"), "padded")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ padded: string; }>>>);
    });
  });
  ...
  describe("initcap(col)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ initcap }) => initcap("User.name"), "titled")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ titled: string; }>>>);
    });
  });
  ...
  describe("strpos(col, substr)", () => {
    ...
    it("type: number", async () => {
      const _result = await prisma.$from("User")
        .select(({ strpos }) => strpos("User.email", "@"), "pos")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ pos: number; }>>>);
    });
  });
  ...
  describe("splitPart(col, delimiter, field)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ splitPart }) => splitPart("User.email", "@", 1), "handle")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ handle: string; }>>>);
    });
  });
  ...
  describe("btrim(col, chars?)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ btrim }) => btrim("User.name"), "t")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ t: string; }>>>);
    });
  });
  ...
  describe("md5(col)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ md5 }) => md5("User.email"), "hash")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ hash: string; }>>>);
    });
  });
