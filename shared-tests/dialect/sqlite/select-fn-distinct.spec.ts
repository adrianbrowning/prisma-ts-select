...
describe("SQLite distinct() helper", () => {
    describe("avg(distinct(col))", () => {
        ...
        it("should match SQL", () => {
            expectSQL(createQuery().getSQL(),
                `SELECT AVG(DISTINCT User.age) AS ${dialect.quote("avgAge", true)} FROM User GROUP BY User.id;`);
        });
    });
    ...
    describe("sum(distinct(col))", () => {
        ...
        it("should match SQL", () => {
            expectSQL(createQuery().getSQL(),
                `SELECT SUM(DISTINCT User.age) AS ${dialect.quote("sumAge", true)} FROM User GROUP BY User.id;`);
        });
    });
    ...
    describe("groupConcat(distinct(col))", () => {
        ...
        it("should match SQL", () => {
            expectSQL(createQuery().getSQL(),
                `SELECT GROUP_CONCAT(DISTINCT User.name) AS ${dialect.quote("names", true)} FROM User GROUP BY User.id;`);
        });
    });
