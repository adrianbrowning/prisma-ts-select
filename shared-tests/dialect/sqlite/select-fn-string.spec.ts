...
describe("SQLite string dialect fns", () => {
  ...
  describe("concat(a, b)", () => {
    ...
    it("type: string", async () => {
      ...
      typeCheck({} as Expect<Equal<typeof _result, Array<{ full: string; }>>>);
    });
    ...
    it("should run and concatenate values", async () => {
      ...
      const nonNull = _result.filter(r => r.full !== null && r.full !== undefined);
  ...
  describe("substr(col, start)", () => {
    ...
    it("type: string", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ s: string; }>>>);
    });
  });
  ...
  describe("substr(col, start, len)", () => {
    ...
    it("should return first 5 chars", async () => {
      const _result = await createQuery().run();
      const values = _result.map(r => r.s).sort((a, b) => String(a).localeCompare(String(b)));
  ...
  describe("composition — lit + column", () => {
    ...
    it("ltrim(rtrim(concat)) — runtime strips whitespace", async () => {
      const _result = await prisma.$from("User")
        ...
        .run();
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition, sonarjs/different-types-comparison
      const names = _result.map(r => r.r).filter((n): n is string => n !== null);
  ...
  describe("length(col)", () => {
    ...
    it("type: bigint", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ elen: bigint; }>>>);
    });

    it("should return correct lengths", async () => {
      const _result = await createQuery().run();
      // eslint-disable-next-line sonarjs/no-nested-conditional
      const lengths = _result.map(r => r.elen).sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
  ...
  describe("instr(col, substr)", () => {
    ...
    it("type: bigint", async () => {
      const _result = await prisma.$from("User")
        .select(({ instr }) => instr("User.email", "@"), "pos")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ pos: bigint; }>>>);
    });
    ...
    it("should return position of @", async () => {
      ...
      const positions = _result.map(r => (r.pos)).sort((a, b) => Number(a - b));
  ...
  describe("char(...codes)", () => {
    ...
    it("type: string", async () => {
      const _result = await prisma.$from("User")
        .select(({ char }) => char(65, 66, 67), "abc")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ abc: string; }>>>);
    });
    ...
    it("should return 'ABC'", async () => {
      const _result = await prisma.$from("User")
        .select(({ char }) => char(65, 66, 67), "abc")
        .run();
      assert.ok(_result.every(r => r.abc === "ABC"));
    });
  });
  ...
  describe("hex(col)", () => {
    ...
    it("should return hex string", async () => {
      const _result = await prisma.$from("User")
        ...
        .run();
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition, sonarjs/different-types-comparison
      const hexVals = _result.map(r => r.h).filter((h): h is string => h !== null && h !== "");
  ...
  describe("unicode(col)", () => {
    ...
    it("type: bigint", async () => {
      const _result = await prisma.$from("User")
        .select(({ unicode }) => unicode("User.name"), "code")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ code: bigint; }>>>);
    });
    ...
    it("should return unicode code point of first char", async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition, sonarjs/different-types-comparison
      const codes = _result.map(r => r.code).filter(c => c !== null)
        .map(c => Number(c));
