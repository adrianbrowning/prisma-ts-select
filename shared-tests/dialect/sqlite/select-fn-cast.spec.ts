...
describe("SQLite cast() fn", () => {
  ...
  describe("cast(year(col), 'INTEGER') — motivating use case", () => {
    ...
    it("returns bigint year values", async () => {
      ...
      const years = _result.map(r => r.y).sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
  ...
  describe("cast(col, 'TEXT')", () => {
    ...
    it("returns string values", async () => {
      const _result = await createQuery().run();
      assert.deepEqual(_result.map(r => r.a).sort((a, b) => String(a).localeCompare(String(b))), [ "1", "2", "3" ]);
    });
  });
  ...
  describe("cast(col, 'REAL')", () => {
    ...
    it("returns float values", async () => {
      const _result = await createQuery().run();
      assert.deepEqual(_result.map(r => r.a).sort((a, b) => a - b), [ 1.0, 2.0, 3.0 ]);
    });
  });
