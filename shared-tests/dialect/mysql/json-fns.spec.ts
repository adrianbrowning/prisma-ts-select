...
describe("MySQL JSON scalar fns", () => {
  ...
  describe("jsonArray(...args)", () => {
    ...
    it("should run and return rows", async () => {
      ...
      // MySQL may return JSON array as string — handle defensively
  ...
  describe("jsonObject(pairs)", () => {
    ...
    it("should run and return rows", async () => {
      ...
      assert.ok(Array.isArray(rows));
      assert.ok(rows.length > 0, "expected rows from Post table");
      assert.deepStrictEqual(Object.keys(rows[0]?.obj ?? {}).sort((a, b) => String(a).localeCompare(String(b))), [ "id", "title" ]);
    });
  });
