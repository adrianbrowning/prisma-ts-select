  ...
  describe("sum(col) — type", () => {
    it("type: Decimal", async () => {
      const _result = await prisma.$from("User").select(({ sum }) => sum("User.age"), "total")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ total: Decimal; }>>>);
    });
    it("runtime value: Decimal(55)", async () => {
      const result = await prisma.$from("User").select(({ sum }) => sum("User.age"), "total")
        .run();
  ...
  describe("avg(col) — type", () => {
    it("type: Decimal", async () => {
      const _result = await prisma.$from("User").select(({ avg }) => avg("User.age"), "average")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ average: Decimal; }>>>);
    });
  });
