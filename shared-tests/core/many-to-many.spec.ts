...
describe("manyToManyJoin", () => {
  ...
  describe("default (unnamed) M2M — Post → Category", () => {
    ...
    it("TFields includes junction + target fields (junction col selectable)", () => {
      ...
      createQuery().select("_M2M_CategoryToM2M_Post.A");
      createQuery().select("M2M_Category.name");
    });
  });
  ...
  describe("safeIdent validation", () => {
    it("throws on unsafe target identifier", () => {
      assert.throws(
        () => prisma.$from("M2M_Post")
        //@ts-expect-error bad join
          .manyToManyJoin("M2M_Post", "M2M_Category; DROP TABLE")
          .getSQL(),
        /unsafe identifier/
      );
    });
    ...
    it("throws on unsafe refName identifier", () => {
      assert.throws(
        () => prisma.$from("MMM_Post")
        //@ts-expect-error bad join
          .manyToManyJoin("MMM_Post", "MMM_Category", { refName: "M2M_NC_M1'; DROP" })
          .getSQL(),
        /unsafe identifier/
      );
    });
    ...
    it("throws on unsafe source identifier", () => {
      assert.throws(
        () => prisma.$from("M2M_Post")
        //@ts-expect-error bad join
          .manyToManyJoin("M2M_Post; DROP TABLE", "M2M_Category")
          .getSQL(),
        /unsafe identifier/
      );
    });
