...
describe("where array shorthand", () => {
  ...
  describe("Stage 1: scalar array → IN", () => {
    ...
    const q = () => prisma.$from("User").where({ "name": [ "John Doe", "John Smith" ] })
      .selectAll();
  ...
  describe("Stage 3: escaping", () => {
    ...
    it("should escape single quotes in scalar array", () => {
      const q = prisma.$from("User").where({ "name": [ "O'Brien" ] })
        .selectAll();
      expectSQL(
        q.getSQL(),
  ...
  describe("Stage 4: empty array guard", () => {
    ...
    it("should throw at runtime on empty scalar array", () => {
      const w = [] as unknown as [string, ...Array<string>];
      assert.throws(
        () => prisma.$from("User").where({ "name": w })
          .selectAll()
          .getSQL(),
    ...
    it("should throw at runtime on empty op-array", () => {
      const w = [] as unknown as [{ op: "LIKE"; value: string; }, ...Array<{ op: "LIKE"; value: string; }>];
      assert.throws(
        () => prisma.$from("User").where({ "name": w })
          .selectAll()
          .getSQL(),
  ...
  describe("Stage 5: single-element op-array — no parens", () => {
    ...
    it("should produce no surrounding parens for single op", () => {
      const q = prisma.$from("User").where({ "name": [{ op: "LIKE", value: "John D%" }] })
        .selectAll();
      expectSQL(
        q.getSQL(),
  ...
  describe("Stage 6: numeric op-array", () => {
    ...
    const q = () => prisma.$from("User").where({ "age": [{ op: "=", value: 25 }, { op: "=", value: 30 }] })
      .selectAll();
  ...
  describe("Stage 10: DateTime scalar array (SQL shape)", () => {
    ...
    it("should produce IN SQL for date array (type check)", () => {
      ...
      const q = prisma.$from("Post").where({ "createdAt": [ d1, d2 ] })
        .selectAll();
  ...
  describe("Stage 11 — Date values in where()", () => {
    ...
    it("op NOT IN with Date values should produce quoted ISO strings", () => {
      const q = prisma.$from("Post")
        .where({ "createdAt": { op: "NOT IN", values: [ fixedDate, fixedDate2 ] } })
        .select("createdAt");
      expectSQL(
        q.getSQL(),
        `SELECT ${dialect.quote("createdAt")} FROM ${dialect.quote("Post")} WHERE ${dialect.quote("createdAt")} NOT IN ('${fixedDateISO}', '${fixedDate2ISO}');`
      );
    });
    ...
    it("op-array of Date values should produce OR chain with quoted ISO strings", () => {
      const q = prisma.$from("Post")
        .where({ "createdAt": [{ op: ">", value: fixedDate }, { op: "<", value: fixedDate2 }] })
        .select("createdAt");
      expectSQL(
        q.getSQL(),
        `SELECT ${dialect.quote("createdAt")} FROM ${dialect.quote("Post")} WHERE (${dialect.quote("createdAt")} > '${fixedDateISO}' OR ${dialect.quote("createdAt")} < '${fixedDate2ISO}');`
      );
    });
...
describe("having", () => {
  ...
  describe("HAVING without GROUP BY", () => {
    ...
    it("should generate SQL (runtime skipped - SQLite limitation)", async () => {
      ...
      const _fields = createQuery().getResultType();

      typeCheck({} as Expect<Equal<typeof _fields, TExpected>>);
