...
describe("Column Alias Support", () => {
  ...
  describe("Aliases with GROUP BY and HAVING", () => {
    ...
    test("should use alias in HAVING clause", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<{ author: PostRow["authorId"]; }>>>);
      }
  ...
  describe("Aliases with LIMIT and OFFSET", () => {
    ...
    test("should work with LIMIT and OFFSET", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<{ emailAddr: UserRow["email"]; }>>>);
      }
  ...
  describe("Edge cases", () => {
    test("should handle special characters in alias names", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<{ user_full_name: UserRow["name"]; }>>>);
      }
  ...
  describe("Duplicate key detection", () => {
    test("should error on duplicate _result keys from same column name", async () => {
