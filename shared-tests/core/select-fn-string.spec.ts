...
describe("select() string fn context", () => {
  ...
  describe("upper(col)", () => {
    ...
    it("type: string", async () => {
      ...
      typeCheck({} as Expect<Equal<typeof _result, Array<{ uname: string; }>>>);
    });

    it("should return uppercase names", async () => {
      const _result = await createQuery().run();
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition, sonarjs/different-types-comparison
      const names = _result.map(r => r.uname).filter(n => n !== null)
        .sort((a, b) => String(a).localeCompare(String(b)));
      assert.deepEqual(names, [ "JOHN DOE", "JOHN SMITH", null ].filter(n => n !== null).sort((a, b) => String(a).localeCompare(String(b))));
    });
  });
  ...
  describe("lower(col)", () => {
    ...
    it("type: string", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ lname: string; }>>>);
    });

    it("should return lowercase names", async () => {
      const _result = await createQuery().run();
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition, sonarjs/different-types-comparison
      const names = _result.map(r => r.lname).filter((n): n is string => n !== null)
        .sort((a, b) => String(a).localeCompare(String(b)));
  ...
  describe("trim(col)", () => {
    ...
    it("type: string", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ email: string; }>>>);
    });
  });
  ...
  describe("replace(col, from, to)", () => {
    ...
    it("should replace substring", async () => {
      const _result = await createQuery().run();
      const handles = _result.map(r => r.handle).sort((a, b) => String(a).localeCompare(String(b)));
  ...
  describe("chaining: upper(lower(col))", () => {
    it("should accept SQLExpr as input", () => {
      const sql = prisma.$from("User")
        .select(({ upper, lower }) => upper(lower("User.name")), "_result")
        .getSQL();
      expectSQL(sql,
        `SELECT UPPER(LOWER(${dialect.quoteQualifiedColumn("User.name")})) AS ${dialect.quote("_result", true)} FROM ${dialect.quote("User")};`);
    });
  });
