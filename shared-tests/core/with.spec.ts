...
describe("$with (CTE)", () => {
  ...
  test("runtime result correctness - filtered CTE", async t => {
    ...
    const inner = prisma.$from("Post").where({ id: 1 })
      .select("id")
      .select("authorId")
      .select("title");
    const result = await prisma
  ...
  test("type check - result shape", () => {
    ...
    type NameType = Awaited<ReturnType<typeof _q.run>>[0]["name"];
    typeCheck({} as Expect<Equal<NameType, string | null>>);
    type TitleType = Awaited<ReturnType<typeof _q.run>>[0]["pp.title"];
  ...
  test("type check - CTE as base result shape", () => {
    const inner = prisma.$from("Post").select("id")
      .select("title");
    const _q = prisma.$with("pp", inner).from("pp")
      .select("pp.id")
      .select("pp.title");

    type IdType = Awaited<ReturnType<typeof _q.run>>[0]["pp.id"];
    type TitleType = Awaited<ReturnType<typeof _q.run>>[0]["pp.title"];
  ...
  test("select * with CTE-in-join and no parseable columns throws", () => {
    const fakeQuery = { values: { selects: [] }, getSQL: () => "" };
    //@ts-expect-error fakeQuery
