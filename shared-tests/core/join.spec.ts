...
describe("join", () => {
  ...
  test("TS join Checks", async t => {

    {
      ...
      const result = await query.select("User.name").select("Post.title")
        .run();
...
describe("*UnsafeTypeEnforced with CTE reference", () => {
  ...
  test("type check - leftJoinUnsafeTypeEnforced accepts CTE column as on-reference", () => {
    ...
    prisma.$with("pp", inner).from("User")
    // @ts-expect-error CTE field "pp.nonexistent" should not exist
      .leftJoinUnsafeTypeEnforced("Post p", "authorId", "pp.nonexistent");
  });
...
describe("join nullability types", () => {
  ...
  test("leftJoin makes new table fields nullable", () => {
    const _q = prisma.$from("User").leftJoin("Post", "authorId", "User.id")
      .select("Post.title");
    type TitleType = Awaited<ReturnType<typeof _q.run>>[0]["title"];
    typeCheck({} as Expect<Equal<TitleType, string | null>>);
  });

  test("leftJoinUnsafeTypeEnforced makes new table fields nullable", () => {
    const _q = prisma.$from("User").leftJoinUnsafeTypeEnforced("Post", "id", "User.id")
      .select("Post.title");
    type TitleType = Awaited<ReturnType<typeof _q.run>>[0]["title"];
    typeCheck({} as Expect<Equal<TitleType, string | null>>);
  });
  ...
  test("leftJoinUnsafeIgnoreType makes new table fields nullable", () => {
    const _q = prisma.$from("User").leftJoinUnsafeIgnoreType("Post", "id", "User.email")
      .select("Post.title");
    type TitleType = Awaited<ReturnType<typeof _q.run>>[0]["title"];
    typeCheck({} as Expect<Equal<TitleType, string | null>>);
  });
  ...
  test("innerJoin preserves field types unchanged", () => {
    const _q = prisma.$from("User").innerJoin("Post", "authorId", "User.id")
      .select("Post.title");
    type TitleType = Awaited<ReturnType<typeof _q.run>>[0]["title"];
  ...
  test("join (default) preserves field types unchanged", () => {
    const _q = prisma.$from("User").join("Post", "authorId", "User.id")
      .select("Post.title");
    type TitleType = Awaited<ReturnType<typeof _q.run>>[0]["title"];
    typeCheck({} as Expect<Equal<TitleType, string>>);
  });
