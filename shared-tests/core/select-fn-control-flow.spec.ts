...
describe("select() fn context — control flow", () => {
  ...
  describe("coalesce()", () => {
    ...
    it("type: string", async () => {
      ...
      typeCheck({} as Expect<Equal<typeof _result, Array<{ contact: string; }>>>);
    });
    ...
    it("should run and return non-null email", async () => {
      ...
      assert.ok(_result.every(r => r.contact !== null && r.contact !== undefined));
    });
  ...
  describe("nullif()", () => {
    ...
    it("type: number | null", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ val: number | null; }>>>);
    });

    it("should run and return null when values equal", async () => {
      const _result = await createQuery().run();
      assert.ok(_result.length > 0);
      assert.ok(_result.every(r => r.val === null));
    });
  ...
  describe("caseWhen() — single branch no else", () => {
    ...
    it("type: string | null", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ bucket: string | null; }>>>);
    });
  });
  ...
  describe("caseWhen() — with else", () => {
    ...
    it("type: string (else eliminates null)", async () => {
      const _result = await createQuery().run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ status: string; }>>>);
    });

    it("should run and categorize users", async () => {
      const _result = await createQuery().run();
      assert.ok(_result.length > 0);
      assert.ok(_result.every(r => r.status === "adult" || r.status === "minor"));
    });
  });
  ...
  describe("cond() — converts WhereCriteria to SQLExpr<boolean>", () => {
    ...
    it("type: boolean", async () => {
      const _result = await prisma.$from("User")
        .select(({ cond }) => cond({ age: { op: ">", value: 0 } }), "flag")
        .run();
      typeCheck({} as Expect<Equal<typeof _result, Array<{ flag: boolean; }>>>);
    });
  });
