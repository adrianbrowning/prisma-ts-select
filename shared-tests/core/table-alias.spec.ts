...
describe("Table Alias Support", () => {
  describe("Single table alias with FROM", () => {
    test("should alias a table with two-parameter syntax", async () => {
...

      {
        ...
        typeCheck({} as Expect<Equal<typeof _result, Array<Pick<UserRow, "name">>>>);
      }
    ...
    test("should work without alias (backward compatibility)", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<Pick<UserRow, "email">>>>);
      }
    ...
    test("should return correct types with alias", async () => {
      const _result = await prisma.$from("User u")
        .select("name")
        .run();
  ...
  describe("Table aliases with joins", () => {
    test("should alias both tables in a join (positional syntax)", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<Pick<UserPostJoinRow, "name"| "title">>>>);
      }
  ...
  describe("Table aliases with GROUP BY and HAVING", () => {
    test("should use aliases in GROUP BY", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<{ "authorId": PostRow["authorId"]; }>>>);
      }
  ...
  describe("Table aliases with LIMIT and OFFSET", () => {
    test("should work with LIMIT", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<{ "name": UserRow["name"]; }>>>);
      }
    ...
    test("should work with LIMIT and OFFSET", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<{ "email": UserRow["email"]; }>>>);
      }
  ...
  describe("Table.* with aliases", () => {
    test("should expand Table.* using alias", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<UserRowQualified<"u">>>>);
      }
    ...
    test("should expand multiple Table.* with aliases in joins", async () => {
      ...
      expectSQL(
        query.getSQL(),
        `SELECT ${dialect.quoteQualifiedColumn("u.id")} AS ${dialect.quote("u.id", true)}, ${dialect.quoteQualifiedColumn("u.email")} AS ${dialect.quote("u.email", true)}, ${dialect.quoteQualifiedColumn("u.name")} AS ${dialect.quote("u.name", true)}, ${dialect.quoteQualifiedColumn("u.age")} AS ${dialect.quote("u.age", true)}, ${dialect.quoteQualifiedColumn("p.id")} AS ${dialect.quote("p.id", true)}, ${dialect.quoteQualifiedColumn("p.title")} AS ${dialect.quote("p.title", true)}, ${dialect.quoteQualifiedColumn("p.content")} AS ${dialect.quote("p.content", true)}, ${dialect.quoteQualifiedColumn("p.published")} AS ${dialect.quote("p.published", true)}, ${dialect.quoteQualifiedColumn("p.createdAt")} AS ${dialect.quote("p.createdAt", true)}, ${dialect.quoteQualifiedColumn("p.authorId")} AS ${dialect.quote("p.authorId", true)}, ${dialect.quoteQualifiedColumn("p.lastModifiedById")} AS ${dialect.quote("p.lastModifiedById", true)}, ${dialect.quoteQualifiedColumn("p.metadata")} AS ${dialect.quote("p.metadata", true)} FROM ${dialect.quote("User")} AS ${dialect.quote("u", true)} JOIN ${dialect.quote("Post")} AS ${dialect.quote("p", true)} ON ${dialect.quoteQualifiedColumn("p.authorId")} = ${dialect.quoteQualifiedColumn("u.id")};`
      );
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<UserPostQualifiedJoinRow<"u", "p">>>>);
      }
    });
  });
  ...
  describe("Edge cases", () => {
    test("should handle aliases with column aliases", async () => {
      ...
      {
        const _result = await query.run();
        typeCheck({} as Expect<Equal<typeof _result, Array<{ "userName": UserRow["name"]; }>>>);
      }
    ...
    test("should preserve data types with aliases", async () => {
      ...
      typeCheck({} as Expect<Equal<typeof _result, Array<Pick<UserRow, "id" | "email">>>>);
      assert.ok(Array.isArray(_result));
    });
