...
describe("Table Alias Support", () => {
    ...
    describe("Table.* with aliases", () => {
        ...
        test("should expand multiple Table.* with aliases in joins", async () => {
            ...
            expectSQL(
                query.getSQL(),
                `SELECT ${dialect.quoteQualifiedColumn("u.id")} AS ${dialect.quote("u.id", true)}, ${dialect.quoteQualifiedColumn("u.email")} AS ${dialect.quote("u.email", true)}, ${dialect.quoteQualifiedColumn("u.name")} AS ${dialect.quote("u.name", true)}, ${dialect.quoteQualifiedColumn("u.age")} AS ${dialect.quote("u.age", true)}, ${dialect.quoteQualifiedColumn("p.id")} AS ${dialect.quote("p.id", true)}, ${dialect.quoteQualifiedColumn("p.title")} AS ${dialect.quote("p.title", true)}, ${dialect.quoteQualifiedColumn("p.content")} AS ${dialect.quote("p.content", true)}, ${dialect.quoteQualifiedColumn("p.published")} AS ${dialect.quote("p.published", true)}, ${dialect.quoteQualifiedColumn("p.createdAt")} AS ${dialect.quote("p.createdAt", true)}, ${dialect.quoteQualifiedColumn("p.authorId")} AS ${dialect.quote("p.authorId", true)}, ${dialect.quoteQualifiedColumn("p.lastModifiedById")} AS ${dialect.quote("p.lastModifiedById", true)} FROM ${dialect.quote("User")} AS ${dialect.quote("u", true)} JOIN ${dialect.quote("Post")} AS ${dialect.quote("p", true)} ON ${dialect.quoteQualifiedColumn("p.authorId")} = ${dialect.quoteQualifiedColumn("u.id")};`
            );
            {
                const result = await query.run();
