...
describe("Select autocomplete validation (issue #120)", () => {
  ...
  describe("Progressive navigation - alias replaces table name", () => {
    test("join alias is used as nav node (string syntax)", () => {
      const q = prisma.$from("User").join("Post p", "authorId", "User.id");
      q.select("p.id");
      q.select("p.title");
      q.select("p.*");
      q.select("User.id");
    ...
    test("join alias rejection: original table name invalid when aliased", () => {
      if (false as boolean) {
        const q = prisma.$from("User").join("Post p", "authorId", "User.id");
        // @ts-expect-error - "Post" is aliased to "p", so "Post.title" is invalid
        q.select("Post.title");
        // @ts-expect-error - "Post.*" invalid when aliased
        q.select("Post.*");
      }
    });
  });
  ...
  describe("Invalid selections - type errors (compile-time only)", () => {
    test("invalid table/field/column are rejected", () => {
      ...
      if (false as boolean) {
        ...
        prisma.$from("User").join("Post", "authorId", "User.id")
    ...
    test("empty string is rejected (triggers autocomplete suggestions)", () => {
      if (false as boolean) {
        // @ts-expect-error - empty string should show progressive suggestions
        prisma.$from("User").select("");
        prisma.$from("User").join("Post", "authorId", "User.id")
        // @ts-expect-error - empty string in join context
          .select("");
      }
    });

    test("partial table prefix without dot is rejected", () => {
      if (false as boolean) {
        // @ts-expect-error - "User" alone (no dot) is not a valid select
        prisma.$from("User").select("User");
        prisma.$from("User").join("Post", "authorId", "User.id")
        // @ts-expect-error - "Post" alone is not valid
          .select("Post");
      }
    });
  });
